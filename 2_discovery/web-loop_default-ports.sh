   ...
   do
      for n in $(grep $m'\/open' $1 | cut -d ' ' -f 2)
         do
            ...
            [ $m = 443 ] && sed -Ei 's/(set target http)s{0,1}(:\/\/.*)/\1s:\/\/'$n'\//g' ~/Tools-n-Stuff/2_discovery/w3af/w3af_script.w3af
            [ $m = 443 ] && sed -Ei 's/(set output_file \.\/.*w3af)(.*_)(Report.html)/set output_file \.\/'$(date +%Y%m%d)'_w3af_'$n'_ssl_\3/g' ~/Tools-n-Stuff/2_discovery/w3af/w3af_script.w3af
            [ $m = 80 ] && sed -Ei 's/(set target http)s{0,1}(:\/\/.*)/\1:\/\/'$n'\//g' ~/Tools-n-Stuff/2_discovery/w3af/w3af_script.w3af
            [ $m = 80 ] && sed -Ei 's/(set output_file \.\/.*w3af)(.*_)(Report.html)/set output_file \.\/'$(date +%Y%m%d)'_w3af_'$n'_\3/g' ~/Tools-n-Stuff/2_discovery/w3af/w3af_script.w3af          
            w3af -s ~/Tools-n-Stuff/2_discovery/w3af/w3af_script.w3af
            [ $m = 443 ] && java -jar ~/Tools-n-Stuff/4_exploitation/TestSSLServer.jar $n 443 >> $(date +%Y%m%d)_TestSSLServer_$n.txt
