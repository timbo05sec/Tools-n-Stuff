...
arinr=$(ls -1 ~/tools/arinr/bin/arininfo 2>&1|grep -v 'No such file or directory');
if [ -z $arinr ];then
...
~/tools/arinr/bin/arininfo -t orgname "$query"|egrep '[0-9]{1,4}\=' >> $file;
echo "Done"
...
for entry in $(egrep '[0-9]{1,4}\=' *_ARININFO_StarSearch.txt|cut -d'(' -f2|cut -d')' -f1); do
	~/tools/arinr/bin/arininfo --data extra -t orghandle $entry >> $file;
	printf '.';
	done
echo "Done"
...
for netname in $(egrep '= NET[6]?-' *_ARININFO_Enumeration.txt|awk '{print $3}'); do
	~/tools/arinr/bin/arininfo --data extra -t nethandle $netname >> $file;
	printf '.';
	done
echo "Done"
